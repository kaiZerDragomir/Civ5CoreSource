...
void CvArmyAI::AddUnit(int iUnitID, int iSlotNum)
{
	...
	int iTurnsToReachCheckpoint;
	CvPlot *pMusterPlot;
	pMusterPlot = GC.getMap().plot(GetX(), GetY());
	if (pMusterPlot)
	{
		iTurnsToReachCheckpoint = TurnsToReachTarget(pThisUnit, pMusterPlot, true /*bReusePaths*/, true, true);
		if (iTurnsToReachCheckpoint < MAX_INT)
		{
