/*	-------------------------------------------------------------------------------------------------------
	(c) 1991-2012 Take-Two Interactive Software and its subsidiaries.  Developed by Firaxis Games.  
...
public:
	...
	void SetTarget(CvPlot *pPlot) {m_pTarget = pPlot;};
	CvPlot *GetTarget() const {return m_pTarget;};

	// Stores extra integer data
	//   For potential upgradeable units stores the unit type since that's a convenient way to sort them
	int GetAuxIntData() {return m_iAuxData;}
	void SetAuxIntData(int iAuxData) {m_iAuxData = iAuxData;}
...
public:
	...
	inline AIHomelandTargetType GetTargetType() {return m_eTargetType;}
	inline void SetTargetType(AIHomelandTargetType eTargetType) {m_eTargetType = eTargetType;}
